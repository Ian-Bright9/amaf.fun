...
### 1.8 Update `resolve_market` for Multi-Option
...
pub struct CreateMarket<'info> {
    #[account(
        init,
        payer = authority,
        space = 8 + 32 + 1 + 1 + 4 + MAX_QUESTION_LENGTH + 4 + MAX_DESCRIPTION_LENGTH
...
export function calculateBuyCost(
  ...
  const collateralNeeded = totalBefore === 0n
    ? newShares * VIRTUAL_LIQUIDITY
    : (totalSharesBefore + newShares === 0n)
      ? 0n
      : (options[targetIndex].collateralBalance * newShares) / (totalSharesBefore + newShares)
...
**Update transaction call to pass options array:**
...
const costPreview = shares > 0n && market
  ? calculateBuyCost(shares, market.options, selectedOption)
  : null
...
1. **Smart Contract - Core Structures**
   ...
   - Update Market and Bet structs
   - Update CreateMarket instruction

2. **Smart Contract - Trading Logic**
   - Implement generalized CPMM in `buy_shares`
...
| Resolution | `winner: bool` | `winner_index: u8` |
| Add Options | Not allowed | Up to 16, dynamic |
| CPMM Formula | YES: NO_shares / total | Price_i: shares_i / total |
